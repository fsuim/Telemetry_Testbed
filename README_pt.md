...
Testbed de telemetria para robótica móvel com simulador MQTT, snapshots Protobuf, gateway SQLite/WAL + WebSocket JSON e UI web estática.
...
```text
src/apps/              entrypoints dos binários
src/app/               orquestração do gateway
src/sim/               simulador, sensores, cache e publisher Protobuf
src/infra/             MQTT, SQLite e WebSocket
include/               headers organizados por camada
proto/                 contratos Protobuf
generated/protobuf-c/  código gerado por protobuf-c
ui/                    interface web estática
scripts/               execução e análise de experimentos
experiments/           bancos, logs e resultados locais
...
```bash
sudo apt-get update
sudo apt-get install -y build-essential libmosquitto-dev mosquitto \
  protobuf-c-compiler libprotobuf-c-dev libsqlite3-dev sqlite3 python3 python3-matplotlib
```
...
Binários gerados no root, preservando compatibilidade:
...
```bash
./telemetry_gateway --db telemetry.db --ws-port 8080 --batch 50 --stats
```

Terminal 3:

```bash
./robot_sim --rate 50 --state-rate 20
```
...
É possível reduzir a duração para teste rápido:
...
A reorganização preserva os comandos principais existentes:
...
O comportamento funcional do fluxo principal não foi alterado.

---

## README original

O conteúdo abaixo foi preservado do README anterior para referência.

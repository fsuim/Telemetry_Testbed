...
static void *imu_thread(void *arg){
    ...
    while(*(a->rt.running)){
        ...
        int rc = mosquitto_publish(cli.mosq, NULL, TELEMETRY_TOPIC_IMU_ACC, (int)sizeof(payload_acc), payload_acc, 0, false);
        ...
        rc = mosquitto_publish(cli.mosq, NULL, TELEMETRY_TOPIC_IMU_GYRO, (int)sizeof(payload_gyro), payload_gyro, 0, false);
        if(rc != MOSQ_ERR_SUCCESS){

...
Telemetry testbed for mobile robotics with an MQTT simulator, Protobuf snapshots, an SQLite/WAL + JSON WebSocket gateway, and a static web UI.
...
```text
src/apps/              binary entrypoints
src/app/               gateway orchestration
src/sim/               simulator, sensors, cache, and Protobuf publisher
src/infra/             MQTT, SQLite, and WebSocket infrastructure
include/               headers organized by layer
proto/                 Protobuf contracts
generated/protobuf-c/  code generated by protobuf-c
ui/                    static web interface
scripts/               experiment execution and analysis
experiments/           local databases, logs, and results
...
Generated binaries are kept in the repository root for compatibility:
...
You can reduce the duration for a quick test:
...
The reorganization preserves the existing main commands:
...
The functional behavior of the main data flow has not been changed.
...
Python is usually already present on Ubuntu. For plotting, you will likely want:

```bash
python3 -m pip install --user matplotlib
```

> If `analyze_gwstats.py` requires additional packages, install them as needed. See “Information pending” at the end.
...
Option B, verbose foreground process:
...
What to expect:
- A SQLite database file named `telemetry.db` appears. WAL mode may create `telemetry.db-wal` and `telemetry.db-shm` while running.
...
What to expect:
- The connection indicator changes to “connected”.
- Telemetry values update live.
- If the UI has a “history” / “last N” option, it should fetch data from SQLite through the gateway.

---

## Sanity Checks (recommended)

### A) Confirm that the simulator is publishing RAW data
...
Interpretation:
- This prints the byte size of a single message from the RAW IMU acceleration topic.
- A non-zero number confirms: **broker reachable + simulator publishing + topic correct**.

### B) Confirm that the snapshot stream exists (Protobuf payload)
...
Interpretation:
- Confirms that a consolidated snapshot is being published on the snapshot topic.

### C) Confirm that the gateway WebSocket is listening
...
- **RAW topics:** sensor-specific binary payloads published on separate MQTT topics. They have minimal overhead and deterministic layout.
- **Snapshot:** a consolidated **Protobuf** message, a “photo of the system state”, published periodically on a single topic. This is what the gateway logs to SQLite and replays to the UI.
...
> If your local checkout differs, for example in topic names or number of motors/sensors, see “Information pending” at the end and adjust accordingly.

---

## Reproducing the Experiment from the Paper Submission (`run_sweep.sh`)

This repository includes an experiment driver script that runs a sweep over snapshot rates and SQLite batching, logs gateway stats, and generates CSV files and figures.

### Prerequisites
- Broker running (Mosquitto)
- Binaries built (`make`)
- Permission to create folders in the repository directory (`logs/`, `db/`, `results/`)
...
- For each configuration:
  - Runs `telemetry_gateway` in the background with `--stats` and logs stderr to a file
  - Runs `robot_sim` for 60 seconds with the chosen snapshot rate
  - Stops the gateway and saves the database and log
- After the sweep:
  - Runs `python3 analyze_gwstats.py ...` to produce CSV files and figures in `results/`
...
- `results/` will contain:
  ...
  - figure files, commonly `*.png`, `*.pdf`, or `*.svg`, depending on the script
...
### Mosquitto will not start / port 1883 is in use
...
- Confirm that the gateway is running:
  ...
  ss -ltnp | grep 8080 || true
  ```
- Confirm that you are using the correct URL in the UI: `ws://localhost:8080`
- If you are opening the UI in Windows while services run in WSL2, `localhost` usually works. If not, try using the WSL2 IP for debugging.
...
### Permissions / paths (`db/`, `logs/`, `results/`)
```bash
mkdir -p db logs results
```

If you cloned into `/mnt/c/...`, consider moving the repository into your WSL home directory to reduce filesystem and permission edge cases.
...
- **Primary target environment:** WSL2 + Ubuntu on Windows 11, with local broker and local processes.
- **Python:** 3.10+ is recommended. Ubuntu default versions are usually fine.
- **Scope:** this setup is designed for controlled local runs. Performance characteristics will differ on real networks because of latency, jitter, and packet loss.

---

## Information pending (placeholders — please do not assume)

If any of these differ in your checkout, they should be clarified or updated for artifact evaluation:

- **Exact CLI defaults used by `run_sweep.sh`:**
  - The script runs `./telemetry_gateway --db ... --batch ... --stats` without explicitly setting `--mqtt-host`, `--mqtt-port`, or `--ws-port`.
  - The script runs `./robot_sim --rate ... --state-rate ...` without explicitly setting `--host` or `--port`.
...
- **Python dependencies for `analyze_gwstats.py`:**
  - Besides `matplotlib`, confirm whether it needs `numpy`, `pandas`, or other packages, and provide `requirements.txt` if available.
- **Final authoritative RAW topic list and payload sizes:**
  - This README lists the main topics. Confirm any additions or removals in your simulator build.
- **Expected figure/CSV filenames in `results/`:**
  - Document the exact filenames once they are stabilized.
